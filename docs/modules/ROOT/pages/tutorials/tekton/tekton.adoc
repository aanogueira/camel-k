...
spec:
  ...
  steps:
    - name: run-integration
      image: aanogueira/camel-k:1.0.0-M2 //<1>
...
<1> The base image for the step is `aanogueira/camel-k:1.0.0-M2`
...
spec:
  steps:
    - name: install
      image: aanogueira/camel-k:1.0.0-M2
      command:
        - kamel
      args:
        - "install"
...
github repository https://github.com/aanogueira/camel-k/tree/main/example/tekton[example/tekton directory].
...
spec:
  ...
  params:
    - name: revision
      value: main
    - name: url
      value: https://github.com/aanogueira/camel-k
----
